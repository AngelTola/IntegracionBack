...
class SSEService {
    ...
    conectarCliente(usuarioId: string, req: Request, res: Response): void {
        ...
        this.clients.set(usuarioId, res);
        console.log(`Usuario ${usuarioId} conectado al SSE`);
        console.log('Usuarios conectados:', this.listarClientesConectados());
    
        // Cerrar conexión cuando el cliente se desconecte
        req.on('close', () => {
            console.log(`Usuario ${usuarioId} cerró la conexión`);
            this.desconectarCliente(usuarioId);
        });
        
        req.on('error', (error) => {
            console.error(`Error en la conexión del usuario ${usuarioId}:`, error);
            this.desconectarCliente(usuarioId);
        });
    }
  
    desconectarCliente(usuarioId: string): void {
    ...
    enviarNotificacion({ evento, data, usuarioId }: { evento: string, data: any, usuarioId: string }): void {
        console.log(`Buscando conexión para el usuario ${usuarioId}`);
        console.log('Usuarios conectados:', this.listarClientesConectados());
        
        const res = this.clients.get(usuarioId);
        
        if (!res) {
            console.log(`No hay conexión para el usuario ${usuarioId}`);
            return; 
        }
        ...
        try {
            ...
            console.log(`Notificación enviada exitosamente al usuario ${usuarioId}`);
        } catch (error) {
    ...
    enviarPing(): void {
        ...
        this.clients.forEach((res, usuarioId) => {
            try {
                res.write(':\n\n');
                totalUsuarios++;
            } catch (error) {
                console.error(`Error al enviar ping al usuario ${usuarioId}:`, error);
                this.desconectarCliente(usuarioId);
            }
        });
